...
class CmajorConan(ConanFile):
    ...
    def source(self):
        git = Git(self)
        git.clone("https://github.com/cmajor-lang/cmajor.git", target=".", args=["--recurse-submodules"])
    ...
    def package(self):
        ...
        # macOS
        copy(self, "**/libCmajPerformer.dylib",
             src=self.build_folder,
             dst=os.path.join(self.package_folder, "lib"),
             keep_path=False)
        # Linux
        copy(self, "**/libCmajPerformer.so*",
             src=self.build_folder,
             dst=os.path.join(self.package_folder, "lib"),
             keep_path=False)
        # Windows
        copy(self, "**/CmajPerformer.dll",
             src=self.build_folder,
             dst=os.path.join(self.package_folder, "bin"),
             keep_path=False)
        copy(self, "**/CmajPerformer.lib",
             src=self.build_folder,
             dst=os.path.join(self.package_folder, "lib"),
             keep_path=False)
